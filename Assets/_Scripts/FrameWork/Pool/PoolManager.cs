...
namespace FrameWork.Pool
{
    public class PoolManager : MonoBehaviour, IInitializable
    {
        ...
        private readonly string _poolDataPath = "PoolData/PoolData";
        public void Init()
        {
            _poolData = (PoolData)ResManager.Instance.GetAssetCache<ScriptableObject>(_poolDataPath);
            if(!_poolData)
                DebugLogger.Log("Pool 初期化失敗");
            dictionary = new Dictionary<GameObject, UnityObjectPool>();
            //例
            //Initialize(enemyPools);
            foreach (var poolCategory in _poolData.allPools)
            {
                Initialize(poolCategory.pools);
            }
        }
